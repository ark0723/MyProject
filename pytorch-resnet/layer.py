...
class PixelUnshuffle(nn.Module):
    ...
    def forward(self, x):
        ...
        [B, C, H, W] = list(x.shape) #[batchsize, channel, height, width]

        x = x.reshape(B, C, H // ry, ry, W // rx, rx) # added two axis
        x = x.permute(0, 1, 3, 5, 2, 4) #순서가 중요! (B, C, ry, rx, H//ry, W//rx) 순으로
...
class PixelShuffle(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(B, C // (ry * rx), ry, rx, H, W) # added two axis
