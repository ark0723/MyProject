...
Original file is located at
    https://colab.research.google.com/drive/1thlsN4agFi6Xt7zO35DZUHhsGsyB-MYI
"""
...
class Compose(object):
    '''
    apply transformation following the order of parameters
    '''
...
class get_anno(object):
    """save json type of annotatoin data"""
...
class add_neck(object):
    '''
    - change the order of coco annotation data; 
    - add 'neck' for annotation
    - neck wil be calculated from two shoulders
    ...
    def __call__(self, meta_data, img, mask_miss):
        ...
        # in MSCOCO dataset
        # right_shoulder[2] == 1, represents (annotatoin O, image O)
        # right_shoulder[2] == 0, represents (annotation 0, image x)
        # right_shoulder[2] == 2, represents (annotatoin x, image x) 
...
class aug_scale(object):
    ...
    def __call__(self, meta_data, img, mask_miss):
        ...
        # randomly change scale (x0.5 ~ x1.1)
        ...
        # scale_abs = np.abs(self.params_transform['target_dist'] / meta_data['scale_provided'])
...
class aug_rotate(object):
    ...
    def __call__(self, meta_data, img, mask_miss):
        ...
        # randomly rotate (-40 degree ~ 40degree)
        dice = random.random()  # (0,1)
        degree = (dice - 0.5) * 2 * self.params_transform['max_rotate_degree']  # degree [-40,40]
        ...
        def rotate_bound(image, angle, bordervalue):
            ...
            return cv2.warpAffine(image, M, (nW, nH), flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_CONSTANT,borderValue=bordervalue), M
        ...
        img_rot, R = rotate_bound(img, np.copy(
            degree), (128, 128, 128))  # fill with blue if there's unfilled pixels in images because of rotation
        mask_miss_rot, _ = rotate_bound(mask_miss, np.copy(degree), (255, 255, 255))

        # update annotation data into roatated positions  
...
class aug_flip(object):
    ...
    def __call__(self, meta_data, img, mask_miss):
        ...
        if doflip:
            ...
            w = img.shape[1]  # img.shape=（width, height）
...
class remove_illegal_joint(object):
    """after data transformation, if a body part position has been changed or disappered, need to update the part's posion"""
    ...
    def __call__(self, meta_data, img, mask_miss):
        ...
        # if the body part out of image frame, part's position will be (1,1,2) 
